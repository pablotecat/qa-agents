...
Every operation on suites requires `project` + `plan_id`. If the user does not know them, run `mcp_ado_testplan / list_plans` first.

## list suites

- **Tool:** `mcp_ado_testplan` / `list_suites`.
...
- **Tool:** `mcp_ado_testplan_test_suite_write` / `create`.
...
- **There is no direct get-by-id tool** in `mcp_ado_testplan_test_suite_*`. Workaround:
  1. `mcp_ado_testplan` / `list_suites` with `project`, `plan_id`.
  2. Filter by `name` or `id` in memory.

## add cases to suite

- **Tool:** `mcp_ado_testplan_test_suite_write` / `add_test_cases`.
- **Args:** `project`, `plan_id`, `suite_id`, `test_case_ids[]` (array of already-created Test Case IDs).
- **Confirmation required:** yes.
- **Important:** this tool does NOT create the Test Cases — it only associates them with the suite. The TCs must already exist (created via `mcp_ado_testplan_test_case_write/create` or already present). To create TCs and associate them with a suite in one go, chain: `mcp_ado_testplan_test_case_write/create` for each TC → then `add_test_cases` with the resulting IDs. This is the closing pattern of the "import test cases from external sources" flow.

## update

- **Tool:** `mcp_ado_wit_work_item_write` / `update` (a "typical" suite is partly managed as a work item of a certain type; structural properties of the suite such as `parent_id` or `suite_type` are not changed via `mcp_ado_wit_work_item_write / update`).
- **Real limitation:** the toolset does not provide a clean tool to re-parent suites or change their type. To rename or change `areaPath`, `mcp_ado_wit_work_item_write / update` may suffice. To restructure, fall back to the portal.
