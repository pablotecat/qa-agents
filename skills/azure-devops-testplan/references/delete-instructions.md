...
1. **Identify the exact target.** Resolve and display: `organization`, `project`, and the `id` AND the `name` of the artefact to delete. For a Suite, also the parent `plan_id`. For a Case, also the parent `suite_id`. If any ID is missing, run `list_plans` / `list_suites` / `list_cases` first — do not guess.
